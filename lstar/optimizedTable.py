...
class TableModule:
    ...
    def __init__(self, ObjectClass, MQModule, A, debugFlag, testFlag):
                ...
                if self.queryRow(s1) == self.queryRow(s2):
                    ...
                    for a in self.A:
                        if self.queryRow(s1+a) != self.queryRow(s2+a):
                            #print("\n" + str(s1.identifier) + " and " + str(s2.identifier))
                            return s1, s2
        return 1
    ...
    def fixTableInconsistent(self, s):
        ...
        for a in self.A:
            for e in self.E:
                if self.getValue(s[0] + a + e) != self.getValue(s[1] + a + e):

                    #print(str(s[0].identifier) + " + " + str((a+e).identifier) + " = " + str(self.getValue(s[0] + a + e)))
                    #print(str(s[1].identifier) + " + " + str((a+e).identifier) + " = " + str(self.getValue(s[1] + a + e)))
                    # add suitable value (ae) to E
                    #print((a+e).identifier)
                    self.E.add(a + e)
                    return 1
        #print("NOT FOUND")
        return 0
